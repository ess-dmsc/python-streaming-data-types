...
- Generate Python bindings for the schema using FlatBuffers' `flatc` executable
  e.g. `flatc --python -o streaming_data_types/name_xx##.py ../streaming-data-types/schemas/xx##_name.fbs`
  `flatc` comes from the `flatbuffers` package, so make sure you have that installed first
...
**\_First update the **version** number in streaming*data_types/\_version.py and push the update to the repository.***
